% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MatLab script for quantization with LAYER discrimination
% *************************************************************************
%
...
[lstm_B_Q,Sb,Z] = Quantize(lstm_B,b);
...
lstm_B_Q_r = deQuantize(lstm_B_Q,Sb,Z);
...
if P==1
    m = min(min(net.Layers(2).Bias));
    for i=0:2^b
        A = m+i*Sb-Sb/2-Sb/6;
        plot(xx,A*ones(length(xx)),'--m','LineWidth',1);
    end
end
...
File = strcat('NetQuantization_LAYER_b',num2str(b),'.png');
saveas(f,File);